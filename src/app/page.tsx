    ...
    const handleChange = (files: FileList | null) => {
        ...
        if (file) {
            if (!file.type.startsWith("image")) {
                ...
                setStatusMessage(`File type: ${fileType} is not supported, select an image instead!`,);
                setChosenImage(null);
                setFileName(null);
            } else {
                ...
                img.onload = function () {
                    ...
                    if (img.naturalHeight < MIN_TILE_SIZE || img.naturalWidth < MIN_TILE_SIZE) {
                        setStatusMessage(`The selected image is too small to process! Width and height must be at least ${MIN_TILE_SIZE}px`,);
                        setChosenImage(null);
                        setFileName(null);
                        return;
                    }
                    if (img.naturalHeight % 2 > 0 || img.naturalWidth % 2 > 0) {
                        setStatusMessage("The image has an odd width or height, it cannot be divided!",);
                        setChosenImage(null);
                        setFileName(null);
                        return;
                    }
    ...
    async function startTiling() {
        ...
        for (let z = maxZoomLevel; z >= 0; z--) {
            ...
            if (scale === 1) {
                ...
                ctx!.drawImage(chosenImage!, 0, 0, imgWidth, imgHeight, 0, 0, scaledWidth, scaledHeight,);
            }
            ...
            for (let x = 0; x < tilesPerSide; x++) {
                for (let y = 0; y < tilesPerSide; y++) {
                    ...
                    tileCtx!.drawImage(canvas, startX, startY, tileSize, tileSize, 0, 0, tileSize, tileSize,);

                    const dataURL = tileCanvas.toDataURL(outputFormat === "png" ? "image/png" : "image/webp", 1.0,);
                    const base64Data = dataURL.replace(/^data:image\/(png|jpg|webp);base64,/, "",);
                    zip.file(`${z}/${y}/${x}.${outputFormat === "png" ? "png" : "webp"}`, base64Data, {
                        base64: true,
                    },);
                    processedTileCount++;
                    setProgress(Math.floor((
                        processedTileCount / totalTileCount) * 100));
    ...
    return (
        <main className="min-h-screen bg-slate-800">
            <div className={"grid justify-center items-start text-white gap-4 p-12"}>
                <div
                    className={"bg-slate-900 flex justify-center items-center text-4xl rounded-lg p-6 gap-4"}
                >
                    <img
                            ...
                            >
                                <div className="grid items-center justify-items-center gap-y-4">
                                    ...
                                    {fileName ? (
                                        ...
                                        <div
                                            className={"grid gap-1 justify-center items-center text-red-400"}
                                        >
                                            {statusMessage != null && (
                ...
                <span>
                  {"This service does not transfer your image to the internet; all operations are local."}
                            ...
                            <li>Each tile has a minimum size of 128px.</li>
                            <li>
                ...
                <div className={"flex justify-center items-center mt-4"}>
                    <div className={"text-sm"}>
                        Developed by
                        <Link
                            className={"inline-flex items-center gap-1 rounded-lg p-1.5 font-bold text-blue-400"}
